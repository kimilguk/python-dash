...
# app.layout = dbc.Container([ # 단일 파일로 실행 할 때
    ...
    dbc.Button("여러 이미지 검출 작업 중...", color="primary", className="me-1", id="btn_search6",
    ...
    html.Div(children=[
            dcc.Markdown(html_table, dangerously_allow_html=True)
        ]),
    html.Div([
...
@callback (
    [Output('tbl6', 'data'),Output('errors6', 'children'),Output("loading-output-6", "children")],
    [Input("btn_search6", "n_clicks")],
    [State('tbl6', 'data'), State('tbl6', 'columns')], # 기존 값 가져오기
